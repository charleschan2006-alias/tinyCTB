...
**信号 = 桌面输入空闲时间**（GNOME Mutter IdleMonitor via D-Bus，xprintidle 兜底；键盘+鼠标全桌面，X11/Wayland 通吃）。实现是**受控轮询非事件驱动**：协调状态全在共享文件 `presence-probe.json`（含上次结果**成败皆记**、全局约定的 nextProbeAt、失败退避），全体 gate 遵守同一节奏（活跃 ~2s、深度空闲 ≥5min 放宽 ~10s、失败退避 2s→…→120s 封顶**绝不永久熄火**）；**flock 租约保证同刻恰有一个探测者**（try_lock_exclusive，锁随进程消亡无陈旧问题），其余 gate 读共享值按经过时间增龄；暂存文件名带 pid 防并发互踩；探测子进程 1s deadline kill。并发测试钉住"8 线程恰探测 1 次"与"失败退避全队生效"。`tinyctb reset` 会清除探测状态文件。真事件驱动（常驻 D-Bus 连接 + active watch）记为后续候选。⚠️最初用的是 pts atime，被 Sol 用真机 pty 实验证伪：raw 模式从端读取键入，atime 纳秒纹丝不动（内核 7.0 实测复现）——我此前的"35 秒不动"只证明了无自输入流量，没证明有输入时会动，间接证据当成了机制验证。无桌面会话（纯 SSH）时 presence 未知，away 标志是唯一开关，`/back` 是确定性释放路径。
...
3. **等待中用户回来（桌面输入苏醒 或 `/back`）→ 原子结算转终端**：已落库的 tap 仍然生效（`expire_or_take_decision` 竞争），否则标 expired、返回不表态、终端弹框；之后再点 TG 按钮收到诚实的"已超时，会话已回到终端"。释放延迟=轮询粒度 100ms + 探测节奏（活跃 ~2s、深度空闲 ≤10s）。★注意终端侧的**可答性**并不依赖这个释放：2026-08-17 pty 实测确认 Claude Code 在 hook 阻塞期间就已渲染自己的权限对话框（对话框出现在 hook 起点而非返回点），释放只决定远程窗口何时关闭。**`/back` 在释放判定里最先检查**，不排在任何桌面探测（及其 1s 超时）之后——不依赖任何桌面接口，SSH 场景也确定有效，延迟即轮询粒度（100ms）。

由此长远程窗口变得无代价：在电脑前对话框始终在终端可答（hook 阻塞期间即已渲染），手机那份随桌面活动交回；不在时窗口一直开着。本机 `approvalTimeoutSeconds` 已配 3600。

测试五条：away 开启即推送（在场也建行也推送，随后才让路）、auto-allow 优先于在场、用户返回即释放（行 expired+曾推送）、`/back` 即释放（无桌面信号）、tap 抢先仍生效；回退各变红。cfg(test) 下 presence 只认 `TINYCTB_TEST_IDLE_MS`——cargo test 跑在 Claude 会话里，真探测会摸到开发者自己的桌面状态污染网关测试。
