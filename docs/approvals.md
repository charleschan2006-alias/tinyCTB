...
1. **人在机器前（15 秒内桌面有输入）→ 网关立刻让路**：终端直接弹框，不建行、不推送、不打扰手机；
2. **键盘空闲 → 远程窗口照常**（推按钮、/threads 可重发、最长可配 24h——clamp 已放宽到 86400）；
3. **等待中用户回来（桌面输入苏醒 或 `/back`）→ 原子结算转终端**：已落库的 tap 仍然生效（`expire_or_take_decision` 竞争），否则标 expired、返回不表态、终端弹框；之后再点 TG 按钮收到诚实的"已超时，会话已回到终端"。释放延迟=轮询粒度 500ms + 探测节奏（活跃 ~2s、深度空闲 ≤10s）。**`/back` 在释放判定里最先检查**，不排在任何桌面探测（及其 1s 超时）之后——不依赖任何桌面接口，SSH 场景也确定有效，延迟即轮询粒度。

由此长远程窗口变得无代价：在电脑前永远是终端体验，不在时窗口一直开着。本机 `approvalTimeoutSeconds` 已配 3600。

测试五条：在场即让路（无行无推送）、auto-allow 优先于在场、用户返回即释放（行 expired+曾推送）、`/back` 即释放（无桌面信号）、tap 抢先仍生效；回退各变红。cfg(test) 下 presence 只认 `TINYCTB_TEST_IDLE_MS`——cargo test 跑在 Claude 会话里，真探测会摸到开发者自己的桌面状态污染网关测试。
