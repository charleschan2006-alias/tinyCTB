...
> **2026-09-03 重构（钩子权威记账，取代下方旧「先投递后记账/刮屏判 Injected」细节）**：真机验暴露了旧设计的因果缺陷——PTY 刮屏无法证明是哪一面关的框，误判会不可恢复地记错答案。现改为：**注入侧只「送键」不记账**（`InjectOutcome::Delivered` = 选择器 chrome 在场、数字+回车已写入活框；回车前再确认一次 chrome 仍在，否则 `Unreachable` 不发回车）；**最终答案一律由 `PostToolUse(AskUserQuestion)` 钩子从 `tool_response.answers` 读出 fork 真正取的选项、权威记账**（`state::record_native_answers_from_hook`，按 thread+question 精确匹配、只填仍开放的当前行、不覆盖历史已答行；`tool_response.answers` 的 key 逐字等于行存的 `question`，2026-09-03 手机注入与本地键盘作答均已实证）。**建新问题时结算掉同 fork 的旧开放 native 行**（`state::settle_stale_native_questions`），保证每个 fork 至多一个开放 native 行，杜绝旧按钮把旧选项打进新问题。`Delivered` 返回独立的 `ApprovalAnswer::Delivered`（callback 审计 `recorded:false`，按钮消费由 `retryable=false` 控制）。已删：刮屏判 Injected、回车后「框消失」判据、`repaint_and_capture`/`flush_pending`、`state::record_native_answer`。下方 2026-09-02 段落为旧设计留档，细节以本注为准。
